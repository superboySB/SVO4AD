...
class Vehicle(RoadObject):
    ...
    @classmethod
    def create_random(cls, v_id, road: Road,
                      speed: float = None,
        ...
        v = cls(v_id, road, lane.position(x0, 0), lane.heading_at(x0), speed)
        return v

    @classmethod
    def create_according_to(cls, v_id, road: Road,
                      speed: float = None,
                      lane_from: Optional[str] = None,
                      lane_to: Optional[str] = None,
                      lane_id: Optional[int] = None,
        ...
        # x0 += offset * road.np_random.uniform(0.9, 1.1)
        # v = cls(v_id, road, lane.position(x0, 0), lane.heading_at(x0), speed)
        v = cls(v_id, road, lane.position(position, 0), lane.heading_at(position), speed)
        return v
