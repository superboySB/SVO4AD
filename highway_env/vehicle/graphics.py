...
class VehicleGraphics(object):
    ...
    def display(cls, vehicle: Vehicle, surface: "WorldSurface",
        ...
        text = font.render(str(v.id), 1, (255, 0, 0))
        center = (vehicle_surface.get_width() / 2, vehicle_surface.get_height() / 2)
        textpos = text.get_rect(center=center)
        vehicle_surface.blit(text, textpos)
    ...
    def get_color(cls, vehicle: Vehicle, transparent: bool = False) -> Tuple[int]:
        ...
        if getattr(vehicle, "color", None):
            ...
            color = cls.BLUE
        elif isinstance(vehicle, MDPVehicle) or isinstance(vehicle, ControlledVehicle) or isinstance(vehicle,
                                                                                                     NGSIMVehicle):
            if vehicle.id == 0:
                color = cls.EGO_COLOR
            else:
