...
class IntersectionEnvSVO(IntersectionEnv):
    @classmethod
    def default_config(cls) -> dict:
    ...
    def _info(self, obs: np.ndarray, action: int) -> dict:
        ...
        info["agents_rewards"] = tuple(self._agent_reward(action, vehicle) for vehicle in self.controlled_vehicles)
        info["agents_dones"] = tuple(self._agent_is_terminal(vehicle) for vehicle in self.controlled_vehicles)
        info["reward_list"] = intersection_ma_reward_list(self.config, self.road)
        return info

    def trial_step(self, actions):

        def _clear_vehicles(road_copy):
            is_leaving = lambda vehicle: "il" in vehicle.lane_index[0] and "o" in vehicle.lane_index[1] \
                                         and vehicle.lane.local_coordinates(vehicle.position)[0] \
                                         >= vehicle.lane.length - 4 * vehicle.LENGTH
            road_copy.vehicles = [vehicle for vehicle in road_copy.vehicles if not (
                                              is_leaving(vehicle) or vehicle.route is None)]
            return road_copy

        def _spawn_vehicle(config, road_copy,
                           np_random,
                           longitudinal: float = 0,
                           position_deviation: float = 1.,
                           speed_deviation: float = 1.,
                           spawn_probability: float = 0.6,
                           go_straight: bool = False) -> None:
            if np_random.rand() > spawn_probability:
                return

            route = np_random.choice(range(4), size=2, replace=False)
            route[1] = (route[0] + 2) % 4 if go_straight else route[1]
            vehicle_type = utils.class_from_path(config["other_vehicles_type"])
            vehicle = vehicle_type.make_on_lane(road_copy, ("o" + str(route[0]), "ir" + str(route[0]), 0),
                                                longitudinal=longitudinal + 5 + np_random.randn() * position_deviation,
                                                speed=8 + np_random.randn() * speed_deviation)
            for v in road_copy.vehicles:
                if np.linalg.norm(v.position - vehicle.position) < 15:
                    return
            vehicle.plan_route_to("o" + str(route[1]))
            vehicle.randomize_behavior()
            road_copy.vehicles.append(vehicle)
        ...
        for v_id, vehicle in enumerate(road_copy.vehicles):
            vehicle.act(self.ACTIONS[actions[v_id]])
        ...
        reward_list = intersection_ma_reward_list(self.config, road_copy)

        road_copy = _clear_vehicles(road_copy)
        _spawn_vehicle(self.config, road_copy, self.np_random, spawn_probability=self.config["spawn_probability"])
        del road_copy

        return reward_list


def intersection_ma_reward_list(config, road):
    def has_arrived(vehicle, exit_distance: float = 25) -> bool:
        return "il" in vehicle.lane_index[0] \
               and "o" in vehicle.lane_index[1] \
               and vehicle.lane.local_coordinates(vehicle.position)[0] >= exit_distance
    ...
    for vehicle in road.vehicles:
        ...
        reward = config["arrived_reward"] if has_arrived(vehicle) else reward
        if config["normalize_reward"]:
            reward = utils.lmap(reward, [config["collision_reward"], config["arrived_reward"]], [0, 1])
...
register(
    id='intersection-svo-v0',
    entry_point='highway_env.envs:IntersectionEnvSVO',
)

# register(
#     id='intersection-masvo-v0',
#     entry_point='highway_env.envs:IntersectionEnvMASVO',
# )
